...
public class RootClass {
	...
	/*
	 * А вот если с Lazy использвать обращение к полям, то упадем по ошибке. Т.к.
	 * JTA транзакция завершена, и уже завершена сессия к БД. Ибо JPA транзакция
	 * живет в рамках JTA. А объясняется это тем, что EJB-компонент motherDAO закрыл
	 * транзацкию после возврата из метода findAny(). Нужно попробовать пометить
	 * этот метод аннотацией Transactional - чисто теоретичеки тогда можно будет и
	 * здесь использовать Lazy
	 */
	/*
	 * @GET
	 * 
	 * @Path("/getMothers")
	 * 
	 * @Produces({"application/json"}) public String getMothers() { List<Mother>
	 * list = motherDAO.findAny(); String result = "["; int count = 0; for (Mother i
	 * : list) { if (count > 0) { result += ","; } result += i.toStringWithPerson();
