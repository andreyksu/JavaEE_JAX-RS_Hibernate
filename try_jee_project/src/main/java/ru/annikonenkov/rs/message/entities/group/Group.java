...
public class Group {
	 ...
	 * create sequence if not exists suid_grenerator as bigint increment 1 start 100;
	 * alter table groups alter column id set default nextva('suid_grenerator');
	 * drop sequence if  exists groups_suid_seq;
	 * 
	 * Вообще не самая плохая идея остановиться на общей последовательности и
	 * GenerationType.IDENTITY
	 * Минус есть: может проявиться при интенсивной вставке (т.к. придётся делать предварительный select на получение id).
	 *
	 * -----------------------------------------------------------------------------
	 ...
	 * Но тут нужно пробовать - т.к. в spring у меня все сработало. Но подсчет велся с 0.
	 * -----------------------------------------------------------------------------
	 ...
	 * По этой причине эту сторону (т.е. Group) помечаем как сторону которой владеют (задав на этой стороне mappedBy, а кто имеет это значение, тот является ведомым, т.е. не располагает внешним ключём).
	 * Еще раз - владелец отношения является тот, кто владеет внешним ключём.
	 * И у Гансалвеса написано что mappedBy нельзя использовать в ManyToOne, что увязывается с вышеприведенной рекамендацией.
	 * 
	 * ------------------------------------------------------------------------
	 * 
