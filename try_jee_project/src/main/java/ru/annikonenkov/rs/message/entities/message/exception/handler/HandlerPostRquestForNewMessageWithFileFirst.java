...
public class HandlerPostRquestForNewMessageWithFileFirst implements iHandlerPostRequestForNewMessageWithFile {
	...
	public byte[] getByteArrayOfFile() throws ExceptionParseRequest {
		...
		int sizeOfListInputPart = inputParts.size();
		// TODO: Возможна потенциальная проблема если будет больше 1го элемента - просто
		// будем перетерать. Нужно отдельно разобраться, возмжно ли больше чем 1элемент.
		if (sizeOfListInputPart > 1) {
			String message = String.format(
					"Для targetParameter = %s у полученного IntupPart размер больше 1 и равен %d. Сообщение создано не будет.",
					MessageFileParameters.File.getParameter(), sizeOfListInputPart);
			log.warn(message);
			// TODO: Возможно здесь стоит выкинуть исключение?
			throw new ExceptionParseRequest("message");
		}
		for (InputPart ip : inputParts) {
